    ...
    const fetchCheckoutId = async () => {
      try {
        ...
        const response = await postRequest('/payment/hyper-pay/prepare-checkout', {
          ...
          brand: selectedBrand || cartData?.data?.payment_method_id,
          }, {}, token, locale);
        ...
        if (checkoutId) {
          ...
          toast.error(t('No checkout ID found in response'));
          setIsLoading(false);
        }
      } catch (error) {
        toast.error(t('Failed to fetch checkout ID'));
        setIsLoading(false);
      }
    };

    if (cartData?.data?.id && selectedBrand) {
      fetchCheckoutId();
    } else {
      // console.log('No cart items or brand found, skipping checkout ID fetch');
    }
    }, [cartData?.data?.id, selectedBrand, cartData?.data?.payment_method_id, token, onPaymentReady]);
            ...
            <div className="w-full">
              ...
              {isLoading ? (
                <div className="flex items-center justify-center p-8">
                  ...
                  <form
                      action={`${url}/checkoutConfirmation/pending?orderId=${cartData?.data?.id}`}
