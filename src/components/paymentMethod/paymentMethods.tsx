...
const PaymentMethods: React.FC<PaymentMethodsProps> = ({ 
  ...
  const { payment_method_id, setPaymentMethodId } = useCheckoutStore();
 ...
 // Run only on mount to auto-select COD
  ...
  const handlePaymentMethodChange = async (method: string) => {
    // Save the payment method ID to the checkout store
    setPaymentMethodId(method);
    
    // Make API request to update cart details with the selected payment method
    ...
    if (cartId && token) {
      try {
        ...
        console.error('Error updating cart details with payment method:', error);
      }
    }
  ...
  // Check if amount to pay is 0
  const amountToPay = Number(cartData?.data?.amount_to_pay) || 0;
  const useWallet = cartData?.data?.use_wallet === true;

  // If amount is 0, don't show payment methods
  if (amountToPay === 0) {
    return (
          ...
          <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
            <div className="flex items-center gap-3">
              ...
              <p className="text-green-800 dark:text-green-200 font-medium">
                {t('You are now paying using wallet')}  
              </p>
            </div>
          </div>
        ) : (
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 ">
      ...
      {showPaymentOptions ? (
            ...
            {paymentOptions.map((option) => (
              <label key={option.value} className="flex items-center p-4 border border-gray-200 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                <input
                  ...
                  name="payment_method"
                  value={option.value}
                  checked={selectedPaymentMethodId === option.value}
        ...
        <div>
            ...
            <div className="flex items-center justify-center p-2 mb-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-600 dark:text-gray-400">
                  {t('Preparing payment with')} {selectedPaymentMethodId?.toUpperCase()}...
                  </span>
              </div>
            </div>
          )}
          
          {selectedPaymentMethodId && selectedPaymentMethodId !== 'cod' && selectedPaymentMethodId !== 'tamara' && selectedPaymentMethodId !== 'tabby' && (
            <div className="mt-4">
              <HyperPayPayment 
                selectedBrand={selectedPaymentMethodId} 
          ...
          {selectedPaymentMethodId === 'tamara' && isProcessingPayment && (
          ...
          {selectedPaymentMethodId === 'tabby' && isProcessingPayment && (
            <div className="flex items-center justify-center p-8">
              <div className="flex items-center gap-3">
                <svg className="w-6 h-6 animate-spin text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
